...
def train(args, model, train_dataset, valid_dataset, thistaskname, onerun, iffuse):
    # total step
    step_tot = (len(
    ...
    for i in range(startepoch, startepoch + args.max_epoch):
        ...
        if args.local_rank in [0, -1]:
            if i >= 0:
                if thistaskname == "summarization":
                    dooneevalforsum(model, valid_dataloader, args, result_dict, optimizer, scaler, i, onerun, iffuse)
                elif thistaskname == "classification":
                    dooneevalforclass(model, valid_dataloader, args, result_dict, optimizer, scaler, i, onerun, iffuse)
                elif thistaskname == "ner":
                    dooneevalforner(model, valid_dataloader, args, result_dict, optimizer, scaler, i, onerun, iffuse)
                else:
                    logger.error("big error!!!!!!!!!!!!!!!!!!")
                    exit - 1
                model.train()
...
def dooneevalforner(modeltoeval,valid_dataloader,args,result_dict,optimizer,scaler,i,onerun, iffuse):
    ...
    if result_dict['val_F1'][-1] > result_dict['best_val_F1']:
        ...
        if not os.path.exists(args.tosavepath + "/" + "NERFUSE"):
            os.mkdir(args.tosavepath + "/" + "NERFUSE")
        if not os.path.exists(args.tosavepath + "/" + "NERFUSE" + "/" + str(onerun)):
            os.mkdir(args.tosavepath + "/" + "NERFUSE" + "/" + str(onerun))
        ...
        if not iffuse:
            model_to_save.model.save_all_adapters(os.path.join(args.tosavepath + "/" + "NER" + "/" + str(onerun)))
        else:
            model_to_save.model.save_adapter_fusion(os.path.join(args.tosavepath + "/" + "NERFUSE" + "/" + str(onerun)), "summarization, classification,ner")


def dooneevalforclass(modeltoeval,valid_dataloader,args,result_dict,optimizer,scaler,i,onerun, iffuse):
    if isinstance(modeltoeval, torch.nn.parallel.DistributedDataParallel):
        model = modeltoeval.module
    else:
        model = modeltoeval
    ...
    if result_dict['acc'][-1] > result_dict['best_acc']:
        ...
        if not os.path.exists(args.tosavepath + "/" + "ClassFUSE"):
            os.mkdir(args.tosavepath + "/" + "ClassFUSE")
        if not os.path.exists(args.tosavepath + "/" + "ClassFUSE" + "/" + str(onerun)):
            os.mkdir(args.tosavepath + "/" + "ClassFUSE" + "/" + str(onerun))
        model_to_save = model.module if hasattr(model, 'module') else model
        # ckpt = {
        #     "promptnumber": model_to_save.promptnumber,
        #     "promptembedding": model_to_save.promptembedding
        ...
        if not iffuse:
            model_to_save.model.save_all_adapters(os.path.join(args.tosavepath + "/" + "Class" + "/" + str(onerun)))
        else:
            model_to_save.model.save_adapter_fusion(os.path.join(args.tosavepath + "/" + "ClassFUSE" + "/" + str(onerun)), "summarization,classification,ner")

def dooneevalforsum(modeltoeval,valid_dataloader,args,result_dict,optimizer,scaler,i,onerun, iffuse):
    if isinstance(modeltoeval, torch.nn.parallel.DistributedDataParallel):
        model = modeltoeval.module
    else:
        model = modeltoeval
    ...
    if result_dict['val_rouge1'][-1] > result_dict['best_val_rouge1']:
        ...
        if not os.path.exists(args.tosavepath + "/" + "SumFUSE"):
            os.mkdir(args.tosavepath + "/" + "SumFUSE")
        if not os.path.exists(args.tosavepath + "/" + "SumFUSE" + "/" + str(onerun)):
            os.mkdir(args.tosavepath + "/" + "SumFUSE" + "/" + str(onerun))
        model_to_save = model.module if hasattr(model, 'module') else model
        # ckpt = {
        #     "promptnumber": model_to_save.promptnumber,
        #     "promptembedding": model_to_save.promptembedding
        ...
        if not iffuse:
            model_to_save.model.save_all_adapters(os.path.join(args.tosavepath + "/" + "Sum" + "/" + str(onerun)))
        else:
            model_to_save.model.save_adapter_fusion(os.path.join(args.tosavepath + "/" + "SumFUSE" + "/" + str(onerun)), "summarization,classification,ner")

def testner(args, test_ner, onerun, iffuse=True):
    ...
    if iffuse:
        model.model.load_adapter(args.tosavepath + "/" + "Sum" + "/" + str(onerun) + '/summarization/')
        model.model.load_adapter(args.tosavepath + "/" + "Class" + "/" + str(onerun) + '/classification/')
        model.model.load_adapter(args.tosavepath + "/" + "NER" + "/" + str(onerun) + '/ner/')
        adapter_setup = Fuse("summarization", "classification", "ner")
        model.model.add_adapter_fusion(adapter_setup)
        model.model.load_adapter_fusion(args.tosavepath + "/" + "NERFUSE" + "/" + str(onerun))
        model.model.set_active_adapters(adapter_setup)
    else:
        model.model.add_adapter("ner")
        model.model.load_adapter(args.tosavepath + "/" + "NER" + "/" + str(onerun) + '/ner/')
        model.model.set_active_adapters("ner")
...
def testclass(args, test_class, onerun, iffuse=True):
    ...
    if iffuse:
        ...
        adapter_setup = Fuse("summarization", "classification", "ner")
        model.model.add_adapter_fusion(adapter_setup)
        model.model.load_adapter_fusion(args.tosavepath + "/" + "ClassFUSE" + "/" + str(onerun))
        model.model.set_active_adapters(adapter_setup)
    else:
        model.model.add_adapter("classification")
        model.model.load_adapter(args.tosavepath + "/" + "Class" + "/" + str(onerun) + '/classification/')
        model.model.set_active_adapters("classification")
...
def testsum(args, test_sum, onerun, iffuse=True):
    ...
    if iffuse:
        ...
        adapter_setup = Fuse("summarization", "classification", "ner")
        model.model.add_adapter_fusion(adapter_setup)
        model.model.load_adapter_fusion(args.tosavepath + "/" + "SumFUSE" + "/" + str(onerun))
        model.model.set_active_adapters(adapter_setup)
    else:
        model.model.add_adapter("summarization")
        model.model.load_adapter(args.tosavepath + "/" + "Sum" + "/" + str(onerun) + '/summarization/')
        model.model.set_active_adapters("summarization")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--tosavepath", dest="tosavepath", type=str,
                        default="t5_adapter_ckpt", help="ckpt dir to save")
    ...
    for onerun in range(startindex, runtimes):
        ...
        args.max_epoch = 1
        for onetaskindex in range(0, 3):
            continue
            touseindex = onetaskindex
            ...
            print("Total # of parameters before adapter: {}".format(n_params))
            model.model.add_adapter(thistaskname)
            model.model.train_adapter(thistaskname)
            model.model.set_active_adapters(thistaskname)
            n_params = sum(p.numel() for p in model.model.parameters())
            print("Total # of parameters after adapter: {}".format(n_params))
            ...
            iffuse = False
            train(args, model, train_dataset, valid_dataset, thistaskname, onerun, iffuse)
            ...
            if args.local_rank in [0, -1]:
                ...
                logger.info("Testing...")
                if thistaskname == "summarization":
                    testsum(args, test_dataset, onerun, iffuse=False)
                elif thistaskname == "classification":
                    testclass(args, test_dataset, onerun, iffuse=False)
                elif thistaskname == "ner":
                    testner(args, test_dataset, onerun, iffuse=False)
                else:
                    logger.error("big error!!!!!!!!!!!!!!!!!!")
                    exit - 1
                logger.info("Finish testing!")

        logger.info("start fuse!")
        args.max_epoch = 1
        for onetaskindex in range(0, 3):
            touseindex = onetaskindex
            ...
            n_params = sum(p.numel() for p in model.model.parameters())
            print("Total # of parameters before adapter: {}".format(n_params))
            for onename in alltaskname:
                # if onename == "ner":
                #     continue
                model.model.add_adapter(onename)
                if onename == "summarization":
                    onepath = 't5_adapter_ckpt/Sum/'+ str(onerun) +'/' + onename + '/'
                elif onename == "classification":
                    onepath = 't5_adapter_ckpt/Class/'+ str(onerun) +'/' + onename + '/'
                else:
                    onepath = 't5_adapter_ckpt/NER/'+ str(onerun) +'/' + onename + '/'
                logger.info(onepath)
                model.model.load_adapter(onepath)
            #["summarization", "classification"]
            #["summarization", "classification", "ner"]
            adapter_setup = Fuse(alltaskname[0], alltaskname[1], alltaskname[2])
            model.model.add_adapter_fusion(adapter_setup)
            model.model.set_active_adapters(adapter_setup)
            model.model.train_adapter_fusion(adapter_setup)
            ...
            logger.info("Start fuse training")

            iffuse = True
            train(args, model, train_dataset, valid_dataset, thistaskname, onerun, iffuse)
            logger.info("Finish fuse training")
            ...
            if args.local_rank in [0, -1]:
                logger.info("Start fuse testing")
                logger.info("Fuse Testing...")
                if thistaskname == "summarization":
                    testsum(args, test_dataset, onerun, iffuse=True)
                elif thistaskname == "classification":
                    testclass(args, test_dataset, onerun, iffuse=True)
                elif thistaskname == "ner":
                    testner(args, test_dataset, onerun, iffuse=True)
                else:
                    logger.error("big error!!!!!!!!!!!!!!!!!!")
                    exit - 1
                logger.info("Finish fuse testing!")
