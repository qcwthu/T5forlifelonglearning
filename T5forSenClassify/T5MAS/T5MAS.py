...
def dooneeval(modeltoeval,valid_dataloader,args,result_dict,optimizer,scaler,i,onerun):
    ...
    if result_dict['acc'][-1] > result_dict['best_acc']:
        ...
        ckpt = {
            't5-large-sc': model_to_save.model.state_dict()
        }
...
def test(args, test_dataset, onerun):
    ...
    model = T5forClassify(args, t5model, test_dataset.tokenizer)
    print(args.tosavepath + "/" + args.taskfold + "/bestckpt")
    allckpt = torch.load(args.tosavepath +  "/" + args.taskfold + "/" + str(onerun) + "/bestckpt")
    model.model.load_state_dict(allckpt['t5-large-sc'])
...
if __name__ == "__main__":
    ...
    for onerun in range(2, 3):
        ...
        #for j in range(1, len(newtaskname)):
            ####run task j
            ###if j == 0, first task
            ###else other task
            ...
            if j == 0:
                ...
                model.model.load_state_dict(t5modeltckpt['t5-large-sc'])
            ...
            logger.info("Calculate MAS")
            t5modeltckptformas = torch.load(args.tosavepath + "/" + newtaskfold[j] + "/" + str(onerun) + "/bestckpt")
            logger.info("load ckpt for mas!!!")
            model.model.load_state_dict(t5modeltckptformas['t5-large-sc'])
            ...
            #print(grads_means)
            #print(grads_fishers)
            logger.info("Finish calculate MAS")
