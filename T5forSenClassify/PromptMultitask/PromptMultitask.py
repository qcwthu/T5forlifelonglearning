...
def train(args, model, train_dataset,valid_dataset, onerun, onestart, oneend):
    # total step
    step_tot = (len(
...
if __name__ == "__main__":
    ...
    #for onerun in range(runtimes):
        ...
        ####first add all generate tokens, add all labels and concat all files
        ##all generate tokens
        ...
        ##all lables: labellist  key+value
        ##concat all files
        trainlist = []
        validlist = []
        testlist = []
        for j in range(0, len(newtaskname)):
            ...
            trainlist.append(thistrainfilename)
            validlist.append(thisvalidfilename)
            testlist.append(thistestfilename)
        alltrainfile = "./Alltrain.txt"
        f = open(alltrainfile, 'w')
        for aa in range(len(trainlist)):
            onefile = trainlist[aa]
            for line in open(onefile, 'r'):
                f.write(str(aa)+ "\t" + line)
        f.close()
        allvalidfile = "./Allvalid.txt"
        f = open(allvalidfile, 'w')
        for aa in range(len(validlist)):
            onefile = validlist[aa]
            for line in open(onefile, 'r'):
                f.write(str(aa) + "\t" + line)
        f.close()
        alltestfile = "./Alltest.txt"
        f = open(alltestfile, 'w')
        for aa in range(len(testlist)):
            onefile = testlist[aa]
            for line in open(onefile, 'r'):
                f.write(str(aa) + "\t" + line)
        f.close()
        ...
        promptembedding = getpromptembedding(model, tokenizer, promptnumber, labellist)
        ...
        train_dataset = T5SenClassifyDatasetConll(alltrainfile, args.max_length, tokenizer, newtgentasktokens,
                                                  answertoken)
        valid_dataset = T5SenClassifyDatasetConll(allvalidfile, args.max_length, tokenizer, newtgentasktokens,
                                                  answertoken)
        test_dataset = T5SenClassifyDatasetConll(alltestfile, args.max_length, tokenizer, newtgentasktokens,
                                                 answertoken)
        ...
        train(args, model, train_dataset, valid_dataset, onerun, allstart[onerun], allend[onerun])
