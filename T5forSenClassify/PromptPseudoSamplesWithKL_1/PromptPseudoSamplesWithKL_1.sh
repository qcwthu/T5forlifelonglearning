...
do
  ...
  do
    ...
    do
        echo "------------------------------"
        python -m torch.distributed.launch --nproc_per_node 1 --master_port 29522 PromptPseudoSamplesWithKL_1.py \
          --cuda 1 \
          --lr $onerate \
          --lm_lambda $onelambda \
          --kd_lamda $onekd \
        ...
        echo "++++++++++++++++++++++++++++++"
        ps aux | grep PromptPseudoSamplesWithKL_1.py | awk '{print $2}' | xargs kill -9
    done
  done
done
