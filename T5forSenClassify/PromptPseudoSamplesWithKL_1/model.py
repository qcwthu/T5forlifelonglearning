...
class T5forClassify(nn.Module):
    def __init__(self, args, model, tokenizer):
        super(T5forClassify, self).__init__()
        self.args = args
        self.model = model
        ### load ckpt
    ...
    def forward(self, batch, ifcalpre):
        ...
        if ifcalpre:
            ...
            if newindex != []:
                ...
                #print(thisdistriuse)
                #print(predistriuse)
                thisdistriuse_sm = self.softmax(thisdistriuse) + sys.float_info.min
                predistriuse_sm = self.softmax(predistriuse) + sys.float_info.min
                #rint(thisdistriuse_sm)
                #print(predistriuse_sm)
