        ...
        for step, batch in enumerate(train_dataloader):
            ...
            if step % args.gradient_accumulation_steps == 0 or step == len(train_dataloader) - 1:
                ...
                if args.local_rank in [0, -1] and global_step % args.log_step == 0:
                    ...
                    logger.info("step: %d, shcedule: %.3f, loss: %.6f, masloss: %.6f" % (
...
if __name__ == "__main__":
    ...
    for onerun in range(2):
        ...
        #for j in range(1, len(newtaskname)):
            ...
            promptckptformas = torch.load(args.tosavepath + "/" + newtaskfold[j] + "/" + str(onerun) + "/bestckpt")
            promptnumberformas = args.prompt_number
            promptembeddingformas = promptckptformas['promptembedding']
            model.set_prompt_embedding(promptnumberformas, promptembeddingformas)
            ###########################################################################
