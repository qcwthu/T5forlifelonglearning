...
def run(args, logger):
    ...
    if args.do_train:
        if args.checkpoint is not None:
            ...
            #model = MyBart.from_pretrained(args.model, state_dict=convert_to_single_gpu(torch.load(args.checkpoint)))
            inermodel = T5ForConditionalGeneration.from_pretrained(args.model, state_dict=convert_to_single_gpu(torch.load(args.checkpoint)))
            model = T5FTModel(args, inermodel)
        else:
            #model = MyBart.from_pretrained(args.model, cache_dir=args.cache_dir)
        ...
        for step, batch in enumerate(train_data.dataloader):
            ...
            if step % args.gradient_accumulation_steps == 0 or step == len(train_data.dataloader) - 1:
                ...
                if args.local_rank in [0, -1] and global_step % args.log_step == 0:
                    logger.info("Step %d Global_Step %d Train loss %.2f on epoch=%d" % (step, global_step, np.mean(train_losses), epoch))
                    train_losses = []
                ...
                if args.local_rank in [0, -1] and global_step % args.eval_period == 0:
                    ...
                    logger.info("Step %d Train loss %.2f %s %s on epoch=%d" % (global_step, np.mean(train_losses_usedforeval), dev_data.metric, curr_em, epoch))
                    train_losses_usedforeval = []
                    if best_accuracy < curr_em:
                        model_state_dict = {k:v.cpu() for (k, v) in model.model.state_dict().items()}
