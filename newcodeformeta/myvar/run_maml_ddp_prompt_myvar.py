...
from dataloader.fewshot_gym_metalearn_myvar import NLPFewshotGymMetaLearningData
...
def train(args, logger, model, train_data, dev_data, optimizer, scheduler):
    ...
    for epoch in range(int(args.num_train_epochs)):
        for batch in tqdm(train_data.dataloader, desc="Epoch {}".format(epoch)):
            ...
            thisdevloss_ori = []
            touseparam = []
            for aa in range(args.reptile_step):
                with higher.innerloop_ctx(
                    model, inner_opt, copy_initial_weights=False, track_higher_grads=False
                ) as (fnet, diffopt):
                    #print(aa * args.inner_bsz,(aa + 1) * args.inner_bsz)
                    train_loss = fnet(input_ids=batch[0][aa * args.inner_bsz:(aa + 1) * args.inner_bsz, :], attention_mask=batch[1][aa * args.inner_bsz:(aa + 1) * args.inner_bsz, :],
                                      decoder_input_ids=batch[2][aa * args.inner_bsz:(aa + 1) * args.inner_bsz, :],
                                      decoder_attention_mask=batch[3][aa * args.inner_bsz:(aa + 1) * args.inner_bsz, :],
                                      is_training=True)
                    ...
                    ###cal dev loss
                    dev_loss = fnet(input_ids=batch[4][aa * args.inner_bsz:(aa + 1) * args.inner_bsz, :], attention_mask=batch[5][aa * args.inner_bsz:(aa + 1) * args.inner_bsz, :],
                                    decoder_input_ids=batch[6][aa * args.inner_bsz:(aa + 1) * args.inner_bsz, :],
                                    decoder_attention_mask=batch[7][aa * args.inner_bsz:(aa + 1) * args.inner_bsz, :],
                                    is_training=True)
                    thisdevloss_ori.append(dev_loss.detach().cpu())
                    ori_param = [p for p in model.parameters() if p.requires_grad]
                    new_param = [p for p in fnet.parameters() if p.requires_grad]
                    #print(len(ori_param),len(new_param))
                    thistouse = (ori_param[0].detach().data - new_param[0].detach().data) / args.inner_lr * (1.0 / args.gradient_accumulation_steps)
                    touseparam.append(thistouse)
                    if stop_training:
                        break
            ...
            allsum = 0.0
            for aa in range(args.reptile_step):
                #print(thisdevloss_ori[aa])
                allsum += np.exp(-thisdevloss_ori[aa])
                #print(allsum)
            for aa in range(args.reptile_step):
                #print(thisdevloss_ori[aa])
                all_coefficient.append(np.exp(-thisdevloss_ori[aa]) / allsum)
                #print(all_coefficient[aa])
                #####softmax


            out_params = [p for p in model.parameters() if p.requires_grad]
            #print(len(out_params))
            for oparam in out_params:
                for aa in range(args.reptile_step):
                    #print(touseparam[aa].shape)
                    if oparam.grad is not None:
                        oparam.grad += all_coefficient[aa] * touseparam[aa]
                    else:
                        oparam.grad = all_coefficient[aa] * touseparam[aa]

            if global_batch % args.gradient_accumulation_steps == 0:   ######maybe we need conduct exp on different gradient_accumulation_steps
                global_step += 1
