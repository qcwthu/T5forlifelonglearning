...
from .utils import MyMetaLearningDatasetforRepitle, MyMetaLearningDataLoader
...
class NLPFewshotGymMetaLearningData(object):
    ...
    def __init__(self, logger, args, data_path, tasks, data_type, is_training):
        ...
        self.data = []
        self.reptile_step = args.reptile_step
    ...
    def load_dataset(self, tokenizer, do_return=False):
        ...
        postfix = tokenizer.__class__.__name__.replace("zer", "zed") + "-" + self.args.identifier
        ...
        self.dataset = MyMetaLearningDatasetforRepitle(train_input_ids, train_attention_mask,
                                        train_decoder_input_ids, train_decoder_attention_mask,
                                        train_metadata_task, train_metadata_questions,
                                        dev_input_ids, dev_attention_mask,
                                        dev_decoder_input_ids, dev_decoder_attention_mask,
                                        dev_metadata_task, dev_metadata_questions,
                                        inner_bsz=self.args.inner_bsz * self.reptile_step,   #####return data for several reptile steps
    ...
    def load_dataloader(self, ifrandom=True, do_return=False):
        self.dataloader = MyMetaLearningDataLoader(self.args, self.dataset, self.is_training, ifrandom)
        if do_return:
