...
from dataloader.fewshot_gym_metalearn_reptile import NLPFewshotGymMetaLearningData
    ...
    for epoch in range(int(args.num_train_epochs)):
        for batch in tqdm(train_data.dataloader, desc="Epoch {}".format(epoch)):
            ...
            with higher.innerloop_ctx(
                ...
                # for name, parameters in fnet.named_parameters():
                #     print(name, ':', parameters.requires_grad)
                # print("**********************************")
                # for name, parameters in model.named_parameters():
                #     print(name, ':', parameters.requires_grad)

                ####after reptile_step, update outer parameters, don't use dev batch
                out_params = [p for p in model.parameters() if p.requires_grad]
                inner_params = [p for p in fnet.parameters() if p.requires_grad]
                #print(len(out_params),len(inner_params))
                # for one in inner_params:
                #     print(one)
                for oparam, iparam in zip(out_params, inner_params):
                    #print((oparam.detach().data - iparam.detach().data))
                    #print(args.inner_lr)
                    #print(1.0 / args.gradient_accumulation_steps)
                    touse = (oparam.detach().data - iparam.detach().data) / args.inner_lr * (1.0 / args.gradient_accumulation_steps)
