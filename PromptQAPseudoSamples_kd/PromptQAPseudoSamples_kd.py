...
def train(args, model, train_dataset,valid_dataset,onerun):
    ...
    for i in range(startepoch, startepoch + args.max_epoch):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            if batchstep % args.gradient_accumulation_steps == 0 or step == len(train_dataloader) - 1:
                if scaler is not None:
...
def dooneeval(modeltoeval,valid_dataloader,args,result_dict,optimizer,scaler,i,onerun):
    ...
    f1s = []
    for (prediction, dp) in zip(allypred, allytrue):
        f1s.append(qa_f1_score(prediction, dp))
    f1score = np.mean(f1s)
    logger.info('----Validation Results Summary----')
    logger.info(len(allypred))
    logger.info(f1score)
    result_dict['val_rouge1'].append(f1score)
    if result_dict['val_rouge1'][-1] > result_dict['best_val_rouge1']:
        logger.info("{} epoch, best epoch was updated! best QAF1: {: >4.5f}".format(i, result_dict['val_rouge1'][-1]))
...
if __name__ == "__main__":
    ...
    #runtimes = 3
    runtimes = 3
    alltaskfold = ["superglue-record", "squad-with_context", "hotpot_qa"]
    alltaskname = ["superglue", "squad",  "hotpot"]
    allgentasktoken = ["QAsuperglue", "QAsquad", "QAhotpot"]
    ...
    #for onerun in range(startindex, runtimes):
        ...
        #for j in range(tostart,len(newtaskname)):
            ...
            if j == 0:
                ...
                logger.info("generate pseodu samples and real samples for previous tasks")
