...
def train(args, model, train_dataset,valid_dataset,test_dataset,grad_mean,grad_fisher,ewc_lambda):
    # total step
    step_tot = (len(
    ...
    for i in range(startepoch, startepoch + args.max_epoch):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            finalloss = loss
            ######add ewc
            ewcloss = param_loss(model, grad_mean, grad_fisher, ewc_lambda, args)
            #logger.info(ewcloss)
            finalloss += ewcloss
            if scaler is not None:
            ...
            if step % args.gradient_accumulation_steps == 0 or step == len(train_dataloader) - 1:
                ...
                if args.local_rank in [0, -1] and global_step % args.log_step == 0:
                    ...
                    #logger.info("step: %d, shcedule: %.3f, loss: %.6f, lmloss: %.6f, ewcloss: %.6f" % (
                    #    global_step, global_step / step_tot, np.average(allloss), np.average(alllmloss), np.average(allewcloss)))
...
def gen_fisher(model, pre_dataloader, lm_lambda):
    fisher = None
    model.eval()
    for step, batch in enumerate(pre_dataloader):
        ...
        grad_params = get_grad_params(model)
        if fisher is None:
            fisher = [param.grad**2/len(pre_dataloader)
                         for param in grad_params]
        else:
            fisher = [fisher[i]+param.grad**2/len(pre_dataloader)
                         for i,param in enumerate(grad_params)]
    model.train()
    return fisher
...
if __name__ == "__main__":
    ...
    print(len(tokenizer))
    genpretasktoken = "__nerco__"
    gentasktoken = "__neronto__"
    answertoken = "__ans__"
    tokenizer.add_tokens(genpretasktoken)
    ...
    train_pre_file_name = "../testmultitask_prompt/conll_fewshot/train.txt"
    pre_lm_lamda = 0.25
    ewc_lambda = 1.0
    #ewc_lambda = 1000.0
    previous_data_set = T5NERDatasetConll(train_pre_file_name, args.max_length, tokenizer, genpretasktoken, answertoken)
    pre_sampler = SequentialSampler(previous_data_set)
    pre_dataloader = get_dataloader(args.num_workers, previous_data_set, 1, args.max_length,
                                      previous_data_set.tokenizer.pad_token_id, pre_sampler)
    grad_params = get_grad_params(model)
    # print(grad_params)
    # print(grad_params[0].shape)
    # print(grad_params[0].requires_grad)
    grad_mean = copy_param_data(grad_params)
    # print(grad_mean)
    # print(grad_mean[0].shape)
    # print(grad_mean[0].requires_grad)
    grad_fisher = gen_fisher(model, pre_dataloader, pre_lm_lamda)
    print(grad_fisher)
    #exit -1

    ######train(args, model, train_dataset,valid_dataset,test_dataset,grad_mean, grad_fisher,ewc_lambda)
    if args.local_rank in [0, -1]:
