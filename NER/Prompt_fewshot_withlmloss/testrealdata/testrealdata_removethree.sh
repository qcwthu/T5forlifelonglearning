...
do
  ...
  do
      echo "------------------------------"
        python -m torch.distributed.launch --nproc_per_node 2 --master_port 29526 testrealdata.py \
          --cuda 3,5 \
          --lr $onerate \
          --lm_lambda $onelambda \
          --weight_decay 1e-5 \
          ...
          --save_step 100000 \
          --eval_step 100000 \
          --save_dir t5ner_real_ckpt_$onelambda \
          --seed 42 \
          --model T5NER \
    ...
    echo "++++++++++++++++++++++++++++++"
    ps aux | grep testrealdata.py | awk '{print $2}' | xargs kill -9
  done
done
