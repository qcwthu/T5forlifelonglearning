...
def getpromptembedding(model,tokenizer,promptnumber):
    ...
    alllabel = ["name entity recognition", "person", "organization", "location", "mix"] ###第二次新的跑的用的这个
    # alllabel = ["name entity recognition", "person", "organization", "location", "money", "country", "time",
    #             "law", "fact", "thing", "measure", "order", "art", "language", "product", "num", "national", "date", "per"]
    for one in alllabel:
...
if __name__ == "__main__":
    ...
    print(len(tokenizer))
    gentasktoken = "__neronto__" #######之前记录了结果的一直用的nerco
    answertoken = "__ans__"
