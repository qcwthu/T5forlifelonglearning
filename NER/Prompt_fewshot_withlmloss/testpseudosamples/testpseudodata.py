...
def train(args, model, train_dataset,valid_dataset,test_dataset):
    ...
    for i in range(startepoch, startepoch + args.max_epoch):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            # print(model.promptembedding)
            # print("-------------------------")
            # print(model.promptembedding_kd.shape)
            # print(model.promptembedding_kd)
            # print("**************************")
            mp_norm = model.promptembedding / model.promptembedding.norm(dim=1)[:, None]
            mpkd_norm = model.promptembedding_kd / model.promptembedding_kd.norm(dim=1)[:, None]
            res = (torch.tensor(1.0) - torch.sum(torch.mul(mp_norm, mpkd_norm), dim=1)).sum(dim=0)
            # print(res.shape)
            # print(res)
            # print("&&&&&&&&&&&&&&&&&&&&&&&&&&")
            finalloss = finalloss * (1.0 - kd_lamda) + res * kd_lamda
...
def getpromptembedding(model,tokenizer,promptnumber):
    ...
    #print(promptinitembedding)
    # alltokens = {}
    fr = open('../testrealdata/allnumber.pickle', 'rb')
