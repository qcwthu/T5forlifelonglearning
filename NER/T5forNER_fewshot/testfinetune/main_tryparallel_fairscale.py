...
def getonebatchresult(sen,target,preds):
    typedic = {"org": "ORG", "location": "LOC", "person": "PERSON", "mix": "MISC"}
    sennum = len(sen)
    restar = []
    respred = []
...
def train(args, model, train_dataset,valid_dataset,test_dataset):
    ...
    #     )
    # #scheduler = get_linear_schedule_with_warmup(optimizer, num_warmup_steps=warmup_steps_total,num_training_steps=step_tot)
    # scheduler = None
    # scaler = None
...
if __name__ == "__main__":
    ...
    if args.model == "T5NER":
        ...
        model = T5forNER(args,tokenizer).to(args.device)
