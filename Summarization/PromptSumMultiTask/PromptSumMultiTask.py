...
if __name__ == "__main__":
    ...
    # for onerun in range(startindex, runtimes):
        ...
        promptembedding = getpromptembedding(model, tokenizer, promptnumber, newtaskname)

        model.set_prompt_embedding(promptnumber, promptembedding)

        model.to(args.device)

        train_dataset = T5SummarizationDataset(alltrainfile, args.max_length, tokenizer, newtgentasktokens, answertoken)
        valid_dataset = T5SummarizationDataset(allvalidfile, args.max_length, tokenizer, newtgentasktokens, answertoken)
        test_dataset = T5SummarizationDataset(alltestfile, args.max_length, tokenizer, newtgentasktokens, answertoken)
