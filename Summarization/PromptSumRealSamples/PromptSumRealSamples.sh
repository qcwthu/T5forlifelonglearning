  ...
  do
    ...
    do
      ...
      do
        ...
        do
              echo "------------------------------"
              python -m torch.distributed.launch --nproc_per_node 1 --master_port 29920 PromptSumRealSamples.py \
                --cuda 2 \
                --lr $onerate \
                --lm_lambda $onelambda \
                ...
                --ifckpt_onlymodel 1
              echo "++++++++++++++++++++++++++++++"
              ps aux | grep PromptSumRealSamples.py | awk '{print $2}' | xargs kill -9
        done
      done
    done
  done
