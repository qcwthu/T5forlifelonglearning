...
if __name__ == "__main__":
    ...
    parser.add_argument("--memnum", dest="memnum", type=int,
                        default=2, help="memory data number")
    ...
    # for onerun in range(startindex, runtimes):
        ...
        # for j in range(tostart,len(newtaskname)):
            ...
            if j == 0:
                ...
                # args.test_file_name = thistestfilename
            else:
                logger.info("get real mem data from previous tasks")
                allpretest = []
            ...
            train_dataset = T5SummarizationDataset(args.train_file_name, args.max_length, tokenizer, newtgentasktokens, answertoken)
            valid_dataset = T5SummarizationDataset(args.valid_file_name, args.max_length, tokenizer, newtgentasktokens, answertoken)
            test_dataset = T5SummarizationDataset(args.test_file_name, args.max_length, tokenizer, newtgentasktokens, answertoken)
            # train_dataset = T5SummarizationDataset(args.train_file_name, args.max_length, tokenizer, gentasktoken,
            #                                        answertoken)
            # valid_dataset = T5SummarizationDataset(args.valid_file_name, args.max_length, tokenizer, gentasktoken,
            #                                        answertoken)
            # test_dataset = T5SummarizationDataset(args.test_file_name, args.max_length, tokenizer, gentasktoken,
            #                                       answertoken)
            ...
            logger.info("Start get mem data of train and valid")
            toselecttrainfilename = dataprefix + thistaskfold + "/" + str(onerun) + "_" + str(args.seed) + "/train.txt"
            toselectvalidfilename = dataprefix + thistaskfold + "/" + str(onerun) + "_" + str(args.seed) + "/valid.txt"
            if not os.path.exists(memdatafolder + "/" + newtaskfold[j]):
                os.mkdir(memdatafolder + "/" + newtaskfold[j])
            trainmempath = memdatafolder + "/" + newtaskfold[j] + "/train_mem.txt"
            validmempath = memdatafolder + "/" + newtaskfold[j] + "/valid_mem.txt"
            memeveryclass = args.memnum
            getmemdata(toselecttrainfilename, trainmempath, memeveryclass)
            getmemdata(toselectvalidfilename, validmempath, memeveryclass)
            logger.info("Finish get mem data of train and valid")
